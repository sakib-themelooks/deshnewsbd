...
<head>
    ...
    <script type="application/ld+json">{"@context": "https://schema.org","@type": "WebSite","url": "{{url('/')}}","potentialAction": {"@type": "SearchAction","target": "{{route('search_result')}}?q={search_term_string}","query-input": "required name=search_term_string"}}</script>
	...
	@yield('css')
	<style type="text/css">.map img{width:100%;height:100%;object-fit:contain}body{top: 0 !important;background:{{config('siteSetting.bg_color')}};font-family:"{{config('siteSetting.currency')}}",Helvetica,Arial,sans-serif;font-size:18px;line-height:1.42857143;color:#333;}
	    @font-face {
          font-family: "Shurjo";
          font-display: swap;
          font-style: normal;
        ...
        .newsdot i {font-size: 10px;margin-right: 10px;}
        .col-md-55 {width: 20%;}
	</style>
...
<body>
    ...
    <script type="text/javascript">function search_bar(e){""!=e?$.ajax({method:"get",url:'{{ route("search_news") }}',data:{q:e},datatype:"text",success:function(e){e?(document.getElementById("search_bar").style.display="block",document.getElementById("show_suggest_key").innerHTML=e):document.getElementById("show_suggest_key").innerHTML="<p>News not found.</p>"}}):document.getElementById("search_bar").style.display="none"}$(document).on("click",function(e){0===$(e.target).closest("#search_bar").length&&$("#search_bar").hide()}),$("#searchKey").on("click",function(){$("#search_bar").show()});</script>
