...
.mix3 .author-image {width: 55px; height: 55px; border-radius: 50%; overflow: hidden;margin-right: 10px;}
.mmt {
    margin-top: 5px;
}
.title2x {
        ...
        <div class="container">
            ...
            <div id="navbar" class="navbar-collapse collapse">
                <ul class="nav navbar-nav">
                    ...
                    <li class="dropdown">
                        <a href="{{ url($menuitem->url) }}" @if(count($menuitem->subMenus)>0) class="dropdown-toggle" data-toggle="dropdown" @endif >{{$menuitem->title}} @if(count($menuitem->subMenus)>0)<b class="caret"></b>@endif </a>
                        @if(count($menuitem->subMenus)>0)
                        <ul @if(count($menuitem->subMenus)>0) class="dropdown-menu multi-level" @endif >
                            @foreach($menuitem->subMenus as $subMenu)                                  
                            <li @if(count($subMenu->childMenus)>0) class="dropdown-submenu" @endif >
                                <a href="{{url($subMenu->url)}}" @if(count($subMenu->childMenus)>0) class="dropdown-toggle" data-toggle="dropdown" @endif >{{$subMenu->title}}</a>
