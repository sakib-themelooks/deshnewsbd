                    ...
                    <div class="col-md-12 col-xs-12 slider2_news_imgs pps videos">
                        ...
                        @elseif($section_item->lazyload)
                            @if($section_news->image)
                                <img class="lazyload" src="{{ asset('upload/images/default.jpg')}}" data-src="{{ asset('upload/images/news/'. $section_news->image->source_path)}}"  alt="{{$section_news->news_title}}">
                            @else
      ...
      <div class="row">
        ...
        @foreach($section_item->newsByCategory->take($section_item->item_number) as $section_news)
                <div class="column slider2_news_imgs pps videos">
                    ...
                    @endif
                    @if($section_news->thumb_url)
                        <img class="lazyload" src="{{ asset('upload/images/default.jpg')}}" data-src="{{$section_news->thumb_url}}"  alt="{{$section_news->news_title}}"  onclick="currentSlide({{$i}})">
                    @elseif($section_item->lazyload)
                        @if($section_news->image)
                            <img class="lazyload" src="{{ asset('upload/images/default.jpg')}}" data-src="{{ asset('upload/images/thumb_img/'. $section_news->image->source_path)}}"  alt="{{$section_news->news_title}}"  onclick="currentSlide({{$i}})">
                        @else
                            <img src="{{ asset('upload/images/default.jpg')}}"  alt="{{$section_news->news_title}}"  onclick="currentSlide({{$i}})">
                        @endif
                    @elseif($section_news->image)
                    ...
                    @else
                    <img class="slide-thumbnail" src="{{ asset('upload/images/default.jpg')}}"  alt="{{$section_news->news_title}}"  onclick="currentSlide({{$i}})">
                    @endif
                </div>
            <?php $i++; ?>
        @endforeach
