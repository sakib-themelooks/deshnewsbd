        ...
        <div class="row">
            ...
            <div class="col-md-6 pps">
                @if($section_item->item_title)
                   ...
                   <a class="dnone-m c13212" href="{{route('category', $section_item->newsByCategory[0]->getCategory->slug)}}">{{$section->sub_title}} <i class="fa fa-arrow-circle-right"></i></a>
                   </div>
                   @endif
                <?php $i = 1; ?>
                <div class="row"> 
                @foreach($section_item->newsByCategory->take($section->item_number) as $section_news)
                    @if($i == 1)
                        <a href="{{route('newsDetails', [$section_news->getCategory->slug, $section_news->id])}}" class="col-md-6 col-xs-12 news-post standard-post244">
                    ...
                    @else
                        <div class="col-md-3 col-xs-6">
                            <a href="{{route('newsDetails', [$section_news->getCategory->slug, $section_news->id])}}" class="news-post standard-post2">
                                <div class="post-gallery">
                                    ...
                                    @if($section_news->type == 3)
                                        <a class="play-link" class="play-link" href="{{route('newsDetails', [$section_news->getCategory->slug, $section_news->id])}}"><i class="fa fa-play-circle-o"></i></a>
                                    @elseif($section_news->type == 4)
                                        <a class="play-link" href="{{route('newsDetails', [$section_news->getCategory->slug, $section_news->id])}}"><i class="fa fa-headphones" aria-hidden="true"></i></a>
                                    @else @endif
                                </div>
