@php
    ...
    ->where('news.status', '=', 'active')->select('news.*', 'categories.category_bd', 'categories.slug', 'media_galleries.source_path')->get();
@endphp
...
<section class="{{($section_item->colmd)}} col-xs-12 pps mix1" style="margin:{{$section_item->margin}};padding:{{$section_item->padding}};">
    ...
    @if($i == 1)
        <a href="{{route('newsDetails', [$section_news->slug, $section_news->id])}}" class="col-md-12 col-xs-12 mmb">
            ...
            <div class="col-md-12 col-xs-12 grid1section_news_title pps" style="background:{{$section_item->bg_text}};color:{{$section_item->bt_text}};padding: 8px;">
                <p>@if($section_news->sub_1)<span class="red">{{$section_news->sub_1}}</span>@endif{{$section_news->news_title}}</p>
            </div>
        </a>
        @else
        <a href="{{route('newsDetails', [$section_news->slug, $section_news->id])}}" class="col-md-{{$section_item->colxs}} col-xs-6 mmb" style="background:{{$section_item->bg_text}};color:{{$section_item->bt_text}};padding: 8px;">
            <div class="col-md-4 col-xs-4 imix videos">
                @if($section_news->news_type)
                <i class="fa {{$section_news->news_type}}" aria-hidden="true"></i>
                @endif
                @if($section_news->thumb_url)
