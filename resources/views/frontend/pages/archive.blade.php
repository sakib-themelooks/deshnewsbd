...
@section('title')
   Archive |  {{Config::get('siteSetting.title')}}
@endsection
...
?>
	...
	<section class="block-wrapper">
		<div class="container section-body">
			<div class="row" >
				<div class="col-md-9 col-xs-12" id="sticky-conent" style=" margin-top:15px;">
				    <span class="archive-title" style="font-size:42px;padding:0.8em 0;color:#000;font-weight: bold;">{{$siteSetting->lang8}}</span>
	                <div class="advertisement">
	                    <div class="desktop-advert">
	                       {!! $topOfNews !!}
	                    </div>
	           		</div>
	           		<form action="{{route('archive')}}" method="get" style="margin:15px 5px">
	           		<div class="row" style="background: #fff;padding: 10px 0;display: flex;align-items: center;">
	           		    
	           		        <div class="col-xs-9 col-md-9">
	           		            <div class="form-group" style="margin-left: 5px;">
    								<select name="day" class="form-control select2" style="width:28% !important;float:left;margin-right:5px">
    								    <option value="">দিন</option>
    								    @for($i=1; $i<=31; $i++)
    								    <option @if(\Request::get('day') == $i) selected @endif value="{{$i}}">{{$i}}</option>
    								    @endfor
    								</select>
    								<select name="month" class="form-control select2" style="width:33% !important;float:left;margin-right:5px">
    								    <option value="">মাস</option>
    								    @for($i=1; $i<=12; $i++)
    								    <option @if(\Request::get('month') == $i) selected @endif value="{{$i}}">{{$i}}</option>
    								    @endfor
    								</select>
    								<select name="year" class="form-control select2" style="width:33% !important;float:left">
	           		        ...
	           		        <div class="col-xs-3 col-md-3">
	           		            <button class="form-control btn btn-success btn-sm">অনুসন্ধান</button>
	           		        </div>
	           		</div>
	           		 </form>
	                @if(count($get_news)>0)
    	                ...
    	                @if($news->getCategory && $news->getCategory->slug)
    						<a href="{{route('newsDetails', [$news->getCategory->slug, $news->id])}}" class="col-md-6 col-xs-12 mmb">
                                ...
                                <div class="col-md-8 col-xs-8 mix77y">
                                    ...
                                    <span style="color:#000;font-size: 14px;display: block; padding-bottom:5px"><i class="fa fa-clock-o"></i>&nbsp; {{banglaDate($news->publish_date)}}</span>
                                </div>
                            </a>
                            @endif
                        @endforeach
				...
				<div class="col-md-3 col-xs-12 div_border last-update">
