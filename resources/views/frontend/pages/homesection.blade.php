          ...
          @if($section->layout_width == 'box')
            <div class="container" style="background:{{$section->background_color}};border-radius:0; padding:{{$section->section_box_mobile}};margin:{{$section->section_box_desktop}};"> @endif
            	...
            	@if($section->section_type != 'category')
            	    @if(View::exists('frontend.homepage.'.$section->section_type.$section->box))
            		@include('frontend.homepage.'.$section->section_type.$section->box)
            		@endif 
            	@else
            	...
            	<?php  
                    $section_items = App\Models\HomepageSectionItem::where('section_id', $section->id)->where('status', 1)->with(['newsByCategory' => function ($query) {
                        $query->where('status', '=', 'active')->orderBy('id', 'desc'); }, 'newsByCategory.image', 'newsByCategory.getCategory:id,category_bd,category_en,cat_slug_en'])
                        ->orderBy('position', 'asc')->get();
                    ?>
                    @foreach($section_items as $section_item)
                        @if(View::exists('frontend.homepage.'.$section_item->section_layout.$section->box))
                		@include('frontend.homepage.'.$section_item->section_layout.$section->box)
                		@endif
                	@endforeach
            	@endif
